...
- `website/styles.css` — single coherent stylesheet, design tokens at the top (`--iron`, `--mortar`, `--brick: #c0231d`, etc.). Fonts via Google Fonts: heavy **Archivo** for display and body (`--display`/`--sans`); **Besley** serif is reserved for the "Construction Inc." lockup lines only (owner rejected serif/italic display type). Small red "string line" dashes (`.stringline` head/pin variants) mark section heads — the full-width hero line and photo caption chip were removed 2026-08-18 at owner request (less busy). Respects `prefers-reduced-motion`.
- `website/script.js` — guarded features (never assume a selector exists): mobile nav toggle, nav scroll shadow, footer year, `.reveal` scroll reveal, and the contact-form fetch handler. The reveal is a deterministic rAF-throttled position check on purpose — an IntersectionObserver missed fast scroll jumps and left sections permanently invisible; don't "modernize" it back.
...
- `assets/branding/print/` — print-ready card artwork (300dpi PNGs with 1/8in bleed): review cards (QR → the Google review link `https://g.page/r/CWqrkfjS4WBDEBM/review`) and general business cards (QR → the website). `README.md` there holds the print spec. QR codes were machine-verified; regenerate the artwork if a link changes, and keep phone numbers off the cards until the business has a dedicated line.
- `api/contact.js` — CommonJS Vercel function; the contact form POSTs JSON to `/api/contact` and it emails the submission via **Resend** (`https://api.resend.com/emails`, no npm deps) from `website@anjconstruction.co` to the owner's Gmail. Honeypot field `company` silently drops bots. Requires the `RESEND_API_KEY` env var on the **anj-website** Vercel project (set 2026-08-18, verified end to end); without it the endpoint returns 500 and the site shows a try-again message. Never put the key in client code.
